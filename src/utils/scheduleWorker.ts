...
self.onmessage = (e: MessageEvent<WorkerMessage>) => {
  ...
  if (data.type === 'START') {
    const { courses, chunkTasks, poolBase, pinned, neededFromPool, options } = data;
    
    let topResults: GeneratedScheduleResult[] = [];
    let evaluatedSchedules = 0;
    let estimatedTotal = 0;
    
    // 1. Pre-filter ALL relevant courses ONCE
    ...
    const evaluateCombination = (courseSet: string[]) => {
      ...
      function backtrack(index: number, currentMask: bigint, currentCombination: Record<string, string[]>, currentSessions: Session[], currentDomains: typeof courseSections) {
        if (index === courseSections.length) {
          ...
          if (options.targets.includes('min_day_gaps')) rawScore -= (metrics.dayGaps * 1000); 
    ...
    const generateCombinations = (currentCombo: string[], startIdx: number) => {
       if (currentCombo.length === neededFromPool) {
          evaluateCombination([...pinned, ...currentCombo]);
          return;
       }
       for (let i = startIdx; i < poolBase.length; i++) {
          currentCombo.push(poolBase[i]);
    ...
    for (const task of chunkTasks) {
       if (neededFromPool === 0 || neededFromPool === task.prefix.length) {
         evaluateCombination([...pinned, ...task.prefix]);
       } else {
